...
void IRC::  handlePRIVMSG(Server& S, Client& client, IRC::command& cmd){
    ...
    if (target[0] == '#' || target[0] == '&') {
        ...
        if (!c) {
            S.sendToClient(client, IRC::makeNumStringName(ERR_NOSUCHCHANNEL, target)); return;
...
void IRC::handleJOIN(Server& S, Client& client, IRC::command& cmd) {
    ...
    while (!names.empty()) {
        ...
        if (sendingMsg.size() + name.size() + 1 > MAX_MESSAGE_LEN) {
...
void IRC::handleINVITE(Server &S, Client &client, command &cmd) {
    ...
    if (!targetChannel) {
        ...
        S.sendToClient(*targetClient, ":" + client.getMask() + " INVITE " + targetClient->getNick() + " :" + targetChannel->getDisplayName());
